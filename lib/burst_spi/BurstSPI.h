...
public:
    ...
    */
    ...
    /** Put 'count' data packets in the SPI TX FIFO buffer
    *
    *  If there is no space in the FIFO buffer it will block until there is space.
    * The FIFO buffer will automatically send the packets. There is no receiving here, only transmitting.
    *
    *  Direct implementation significaly faster then 
    *  for(i=0; i < count; ++i) fastWrite(data);
    *  it takes about 10% of speed (tested on stm32f103c8 with toolchain-gccarmnoneeabi 1.70201.0 (7.2.1))
    * 
    *  @param data Data to be sent to the SPI slave
    *  @param count Count of repeats 'data'
    */
    void fastWriteN(uint16_t data, uint32_t count);
