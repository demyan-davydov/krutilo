#pragma once
#ifndef file__BRUSH_STRIP_HORZ_H
#define file__BRUSH_STRIP_HORZ_H

#include "brush.h"
#include "geo/gc_point.h"

class GCBrushStripHorz : public GCBrush
{
    uint16_t clr[2];
    uint8_t len1, len2;
    uint8_t period;
...
public:
    GCBrushStripHorz(uint16_t c1_, uint16_t c2_, uint8_t len1_, uint8_t len2_, const GCPoint& offset_ = {0,0}) 
    ...
    virtual GCBrush* clone() const { return new GCBrushStripHorz(*this); }
};

#endif // file__BRUSH_STRIP_HORZ_H